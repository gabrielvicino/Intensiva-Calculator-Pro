...
# --- FUNÇÃO DE FORMATAÇÃO (Atualizada para aceitar casas decimais variáveis) ---
def format_br(valor, casas=1):
    """
    Formata número para padrão BR (vírgula decimal, ponto milhar).
    casas: número de casas decimais (padrão = 1).
    """
    if valor is None: return ""
    if isinstance(valor, (int, float)):
        # Cria a string de formatação dinamicamente, ex: "{:,.2f}"
        fmt = f"{{:,.{casas}f}}"
        texto = fmt.format(valor).replace(",", "X").replace(".", ",").replace("X", ".")
        return texto
    return str(valor)
...
def calcular_infusao():
    ...
    if droga_nome:
        ...
        with c_lim2:
            # ALTERADO AQUI: Label para MÁXIMA HABITUAL
            st.markdown(f"""<div class="result-box" style="border-left-color: #2ecc71;"><div class="result-title">MÁXIMA HABITUAL<br>({format_br(info['dose_max_hab'])})</div><div class="result-value">{format_br(bomba_max_hab)} ml/h</div></div>""", unsafe_allow_html=True)
        with c_lim3:
            # ALTERADO AQUI: Label para MÁXIMA ESTUDADA
            st.markdown(f"""<div class="result-box" style="border-left-color: #e74c3c;"><div class="result-title">MÁXIMA ESTUDADA<br>({format_br(info['dose_max_tol'])})</div><div class="result-value">{format_br(bomba_max_tol)} ml/h</div></div>""", unsafe_allow_html=True)
        ...
        with col_sim1:
            ...
            if ml_h_atual > 0:
                ...
                # ALTERADO AQUI: Texto de erro/alerta
                if dose_real > float(info['dose_max_tol']): st.error("🚨 PERIGO: Dose acima da MÁXIMA ESTUDADA!")
                elif dose_real > float(info['dose_max_hab']): st.warning("⚠️ Atenção: Dose acima da máxima habitual.")
...
# ==============================================================================
# PÁGINA: CONVERSÃO (LAYOUT DETALHADO)
# ==============================================================================
def page_conversao():
